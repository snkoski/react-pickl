...
const fetchGameTeamStatsStart = (state, action) => {
    return { ...state, error: null, loading: true };
};

const fetchGameTeamStatsSuccess = (state, action) => {
    console.log("IN SUCCESS", action)
    return { ...state, home: action.data.home, away: action.data.away, error: null, loading: false };
};

const fetchGameTeamStatsFail = (state, action) => {
